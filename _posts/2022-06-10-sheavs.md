...
**Theorem 5.1.4.** (Hilbert's Nullstellensatz).    
...
**Definition 5.1.5.** Let $X$ be an affine variety of $k^n$. A **polynomial function** on $X$ is a map $X \to k$ where $x \mapsto f(x)$ for some $f \in k[x_1, \ldots, x_n]$. The ring of all polynomial functions on $X$ is just the quotient ring \\[ A(X):= k[x_1, \ldots, x_n] / I(X). \\]   We usually call $A(X)$ the **coordinate ring** of the affine variety $X$. 



## 5.2. Irreducibility

**Definition 5.2.1.** A closed set $X \subseteq k^n$ is **irreducible** if $X$ cannot be written as a union of proper closed subsets of $X$; that is, if $X = X_1 \cup X_2$ with $X_1, X_2$ being closed sets of $k^n$, then $ $X =X_1$ or $X = X_2$.
...
**Remark 5.2.4.** The ring $k[x_1, \ldots, x_n]$ is Noetherian, so an algebraic set $X \subset k^n$ can be written uniquely as a finite union of irreducible algebraic sets, called the *irreducible components* of $X$.

## 5.3 Rational functions
Throughout this section, fix an irreducible set $X$.

**Definition 5.3.1.** Let $X$ be an irreducible algebraic set, with regular functions/coordinate ring $k[X]$. The fraction field of $k[X]$, denoted $k(X)$, is called the **field of rational functions on $X$**. \\[ K(X) = \left \\{ \frac{g}{h} \colon g, h \in k[X], h \neq 0 \right \\} \\]