...
**Definition 5.3.1.** Let $X$ be an affine variety, and let $U$ be an open subset of $X$. A **regular function** on $U$ is a map $\varphi \colon U \to k$ with the following property: for every $a \in U$, there are polynomial functions $f, g \in A(X)$ with $f(x) 