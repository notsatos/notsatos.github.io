...
Sheaves and sheaf were in invented by Jean Lerray while he was a war prisoner in Austra— following the WWII trend of doing advanced mathematics in prisons (Weil and the Riemann Hypothesis over finite fields). Essentially, the point of a sheaf is to keep track of local algebraic data for some topological space. For example, an algebraic variety is a space that looks locally like the zero set of polynomials, and tthe regular functions on open subsets of a variety form a sheaf.
...
**Proposition 1.1.** Let $X, Y$ be topological spaces. For each open $U \subset X$, define the set of continuous maps as \\[ C(U, Y) = \\{f \colon U \to Y \colon f \text{ is continuous}\\}, \\] and for each inclusion $ U \subset V$, define $\rho_{V U } \colon C(V, Y) \to C(U, Y)$, where $\rho_{V, U} (f) = f \mid _U$. Then $C$ is a sheaf.

*Proof.* Let $ U \subset X$. If $\rho _{U, U} \mathscr{F} (U) \to \mathscr{F}(U)$, then $f \mapsto f \mid _{U}$, but this is just $f$ itself again as $f \colon U \to Y$, so then $\rho _{U, U} = \id _{\mathscr{F} (U)}$. Suppose $W \subset V \subset U$. Now let $q \in \mathscr{U}$. Then $\rho _{U, V} \colon \mathscr{F}(U) \to \mathscr{F}(V)$ gives us that $f \mapsto f \mid_{V}$, and if we map $f \mid_{V}$ to $\mathscr{F} (W)$, then $f \mid _{V} \mapsto (f \mid_{V}) \mid_{W}$, which is clearly just $f \mid_W$. This is to say that $\rho _{U W} \colon \mathscr{F}(U) \to \mathscr{F} (W)$, where $f \mapsto f \mid_{W}$, is the same as $\rho_{UW} \circ \rho _{UV}$.
...
Let $\mathscr{F}$ be a sheaf on $X$, and let $ x \in X$. We say that $s_1 \in \mathscr{F}(U_1), s_2 \in \mathscr{F}(U_2),$ where $U_1, U_2$ are open neighborhoods of $x$, equivalent, if there exists an open neighborhood $V$ of $x$ contained in $x$ such that $\rho_{U_1, V} (s_1) = \rho_{U_2, V} (s_2)$. If we are given a sheaf $\mathscr{F}$ of sets on topological space $X$, then we can consider at every point $x \in X$ for which there is an associated set $\mathscr F_x$ called the stalk of $\mathscr F$ at $x$. Furthermore, the stalk of $\mathscr F$ at $x$ is as a colimit of all $\mathscr F (U)$ over all open sets $U$ containing $x$:
