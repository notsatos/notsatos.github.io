...
An alternative way to define the stalk at a point $p$ is define it in a categorical manner: If $\mathfrak{F}$ is a (pre)sheaf on $X$ and $p \in X$ is a point, then we define the stalk of $\mathscr{F}$ at p to be the direct limit \\[ \mathscr{F}_p = \varinjlim _{p \in U} \mathscr{F} (U) \\] 
...
Moreover, using this construction of a stalk as above with a direct limit, if $p \in U$ and $f \in \mathscr{F} (U)$, then the image of $f$ in $\mathscr{F}_p$ is called the *germ* of $f$ at $p$.
