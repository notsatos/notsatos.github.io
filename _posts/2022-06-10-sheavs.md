...
A morphism of presheaves $\varphi$ is called *injective* if for every open $U$ of $X$, $\varphi _U$ is injective, where $\varphi _U \colon \mathscr{F} (U) \to \mathscr{G} (U)$.  
An *isomorphism* is an invertible morphism $\varphi$. That is, $\varphi _U$ is an isomorphism for every open $U$ of $X$.   
For any $x \in X$, $\varphi$ canonically induces a morphism $\varphi _x \colon \mathscr{F} _x \to \mathscr{G} _x$ such that $ (\varphi_U (s)) _x = \varphi _x (s_x)$ for any open $U$ of $X$, with $s \in \mathscr{F} (U)$, and $x \in X$ (i.e. $s$ is a germ of $U$)—see Section 2. for clarification on this.
