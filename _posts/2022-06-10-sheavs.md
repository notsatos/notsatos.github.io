...
**Proposition 3.1.** Let $\mathscr{F}$ be a presheaf on $X$. Then there exists a sheaf $\mathscr{F}^+$ together with a morphism $\theta \colon \mathscr{F} \to \mathscr{F}^+$ such that if $\varphi \colon \mathscr{F} \to \mathscr{G} $ is a any morphism and $\mathscr{G}$ is a sheaf, then there is a unique morphism $\Phi \colon \mathscr{F}^+ \to \mathscr{G}$ such that the following diagram commutes
...
*Proof.* We will define the sheaf $\mathscr{F}^+$ as follows. Let $U \subset X$ be open. We let $\mathscr{F}^+ (U)$ be defined as the set of elements $\sigma \in \prod _{x \in U} \mathscr{F}_x$ such that for each $x \in U$ there is an open neighborhood $V$ of $x$ contained in $U$ and a section $s \in \mathscr{F} (V)$ such that $\pi _y (\sigma) = s_y$ for all $y \in V$, where $\pi_y$ is the obvious projection map..sa
