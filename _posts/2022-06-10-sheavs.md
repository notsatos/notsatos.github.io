...
## 2. Stalks & Germs
We define the *stalk* of a (pre)sheaf at a point $p$ to be the set of *germs* of $\mathscr{F}$ at $p$, denoted $\mathscr{F}_p$. In a word, germs correspond to sections over some open set containing $P$, where a pair of these section are considered *equivalent* if they agree on some smaller open set. This to say, in a more precise manner, that the stalk at a point $p$ is \\[ \\{ (f, \text{open } U \colon p \in U, f \in \mathscr{F}(U) \\} \\] modulo the relation (on germs) that $(f, U) \sim (g, V)$ if there is some open set $W \subset W, V$ such that $p \in W$ and $\rho _{UW} (f) = \rho _{VW} (g)$.   

An alternative way to define the stalk at a point $p$ is define it in a categorical manner: If $\mathfrak{F}$ is a (pre)sheaf on $X$ and $p \in X$ is a point, then we define the stalk of $\mathscr{F}$ at p to be the direct limit of all $\mathscr{F}(U)$ over all open $U$ containing $p$ (additionally, if $p \in U$ and $f \in \mathscr{F} (U)$, then the image of $f$ in $\mathscr{F}_p$ is called the *germ* of $f$ at $p$): \\[ \mathscr{F}_p = \varinjlim _{p \in U} \mathscr{F} (U) \\] 
...
*Proof.* If $s_x = t_x$ for all $x \in U$, then for each $x \in U$ there is some open subset $U_x \subset U$ containing $x$ such that $\rho_{U U_x} (s) = \rho_{U U_x} (t)$, and as $\mathfrak{U} = \\{ U_i\\} _{i \in I}$ is an open cover of $U$, then the locality/identity axiom of a sheaf (which makes our presheaf into a seperated presheaf) implies that $s =t$. ▨

***Remark 2.1.*** A map $\varphi \colon \mathscr{F} \to \mathscr{G}$ between two presheaves $\mathscr{F}$ and $\mathscr{G}$ on a topological space $X$ induces a map of stalk $\varphi_x \colon \mathscr{F}_x \to \mathscr{G}_x$ for all $x \in X$. To define $\varphi_x$ we proceed as follows. Any element $\gamma \in \mathscr{F}_x$ is an equivalence class $\gamma = s_x$ for some section $s \in \mathscr{F} (U)$ and some open $U \subset X$ containing $x$. Now let $\varphi_x(s_x) = (\varphi_U (s))_x$, where $\varphi_U \colon \mathscr{F}(U) \to \mathscr{G}(U)$ is the map defining $\varphi$ on $U$. As an exercise, we leave for the reader to show that this mapping is well defined.

**Proposition 2.1.** Let $\varphi \colon \mathscr{F} \to \mathscr{G}$ be a morphism of sheaves on a topological space $X$. Then $\varphi$ is an isomorphism if and only if the induced map on stalk $\varphi_p \colon \mathscr{F}_p \to \mathscr{G}_p$ is an isomorphism for every $p \in X$.
