...
**Definition 5.1.** Given an ideal $I$ of $k[x_1, \ldots, x_n]$, the vanishing set of $I$ is \\[ V(I) = \\{ (a_1, \ldots, a_n) \in k^n \colon f(a_1, \ldots, a_n) = 0 \text{ for all $f \in I$} \\} \\]
...
(b) If $\mathfrak a$ and $\mathfrak b$ are two ideals of $k[x_1, \ldots, x_n ]$, then $V(\mathfrak {ab}) = V (\mathfrak{a}) \cup V( \mathfrak b)$.      
...
**Definition 5.2.** Let $X = V(I)$ be a Zariski closed set of $k^n$. The **ideal of $X$** is \\[ I(X) = \\{ f \in k[x_1, \ldots, x_n] \colon f(x) = 0 \text{ for all $x \in X$} \\} \\]

We can note that $I(X)$ is indeed an ideal, and that $I \subseteq I(X)$ but they are note necessarily equivalent, e.g. if we consider $k[x]$ and take $I= (x^2)$, then $X = \\{ 0 \\}$ and $I(X) = (x)$. In fact, it is true that $I(X) = \sqrt{I}$, where the $\sqrt{I} = \\{ f \in k[x_1, \ldots, x_n ] \colon f^m \in I \text{for some m \in \zz _{\geq 0} } \\}$.