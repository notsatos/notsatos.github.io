...
**Definition 5.1.6.** Let $X$ be an affine variety of $k^n$. A **polynomial function** on $X$ is a map $X \to k$ where $x \mapsto f(x)$ for some $f \in k[x_1, \ldots, x_n]$. The ring of all polynomial functions on $X$ is just the quotient ring \\[ A(X):= k[x_1, \ldots, x_n] / I(X). \\]   We usually call $A(X)$ the **coordinate ring** of the affine variety $X$. 
